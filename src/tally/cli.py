...
Grouped by what you are trying to find out:
    ...
    tally doctor              what is configured and what is not
    tally scenarios           the platform-abstraction cost table
    tally tools               tool-representation token costs
    tally dataset build|stats the ground-truth corpus
    tally baseline            the zero-model floor
    tally run                 one case, one arm, verbose
    tally eval                one arm over the corpus
    tally ablate              every arm, then the comparison table
"""
...
def _require_usable_providers(registry, *, allow_paid: bool) -> None:
    ...
    """
    from tally.models.base import Purpose
    ...
    console.print(
        ...
        "(TALLY_GEMINI_API_KEY, TALLY_GLM_API_KEY, TALLY_GROQ_API_KEY, "
        "TALLY_CEREBRAS_API_KEY, TALLY_SILICONFLOW_API_KEY), or run "
        "[bold]ollama serve[/bold] with qwen3:8b pulled for the local tier.\n"
        "Run [bold]tally doctor[/bold] to see what is reachable, and "
        "[bold]tally baseline[/bold] for the zero-model floor, which needs no "
...
def doctor() -> None:
    ...
    cfg = settings()
    console.print(f"[bold]tally {__version__}[/bold]")
...
def tools(scenario: str = typer.Option("dd_finance", help="Scenario to inspect.")) -> None:
    ...
    if scenario == "dd_finance":
        ...
        from tally.scenarios.bi_analyst.spec import build_tools

        registry = build_tools(cfg.paths.data / "example.db")
    elif scenario == "code_engineer":
        from tally.scenarios.code_engineer.spec import build_tools
...
def dataset_build(
    ...
    from tally.scenarios.dd_finance.corpus import CORPUS
    from tally.scenarios.dd_finance.groundtruth import build_dataset, save_dataset
    from tally.scenarios.dd_finance.sec_client import SecClient
...
def dataset_stats() -> None:
    ...
    if not summary_path.exists():
        console.print("[red]no dataset built[/red] — run `tally dataset build`")
        raise typer.Exit(1)
...
def eval(  # noqa: A001 - the command really is called eval
    ...
    workers: int = typer.Option(1, help="Cases in flight at once."),
) -> None:
    """Run one ablation arm over the corpus and save its scores."""
    from tally.evaluation.harness import Harness
    from tally.evaluation.runner import ARMS_BY_NAME, RunnerContext, make_case_runner
    from tally.models.registry_default import build_registry, describe_registry
    from tally.scenarios.dd_finance.sec_client import SecClient
    ...
    if not cases:
        console.print("[red]no cases[/red] — run `tally dataset build` first")
        raise typer.Exit(1)
    ...
    result = harness.run(make_case_runner(context), progress=True, workers=workers)
...
def ablate(
    ...
    for config in selected:
        ...
        result = Harness(cases, arm=config.name, level=level,
            ...
            make_case_runner(context), progress=True, workers=workers)
