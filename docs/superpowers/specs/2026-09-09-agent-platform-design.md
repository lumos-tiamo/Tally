...
> **这是设计时的原始文档，不是交付状态。** 实际交付、实测数字与未完成项见
> [`README.md`](../../../README.md)。两者刻意分开：本文档记录当初的判断，
> README 记录实际发生的事，其中包括几处设计时没预料到的问题
> （XBRL 的 `fy` 语义、submissions 分页、资产负债表恒等式的 NCI 口径、
> Docker Desktop 的文件共享行为），以及一处未兑现的承诺（A 股语料）。
> 第 9 节的占位数字已由 [`docs/resume.md`](../../resume.md) 中的实测值替代。
