...
``bridged``
    ...
    :mod:`tally.execution.bridge`, so the sandbox stays on ``--network none``
...
from tally.context.retrieval import Doc, Retriever, build_retriever
...
class ToolRegistry:
    ...
    def stub_source(self, module: str) -> str:
        from tally.execution.stubgen import render_module  # local import: cycle
    ...
    def materialise(self, tools_dir) -> list[str]:
        from tally.execution.stubgen import write_package  # local import: cycle
