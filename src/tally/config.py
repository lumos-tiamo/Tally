...
class Settings:
    ...
    def load(cls) -> "Settings":
        ...
        return cls(
            paths=Paths().ensure(),
            sec_user_agent=_env("TALLY_SEC_USER_AGENT"),
            ollama_base_url=_env("TALLY_OLLAMA_BASE_URL", "http://localhost:11434"),
            allow_paid=_flag("TALLY_ALLOW_PAID", False),
            redis_url=_env("TALLY_REDIS_URL"),
            api_token=_env("TALLY_API_TOKEN"),
            api_host=_env("TALLY_API_HOST", "127.0.0.1"),
            api_port=int(_env("TALLY_API_PORT", "8000") or 8000),
            api_workers=int(_env("TALLY_API_WORKERS", "1") or 1),
            feishu_app_id=_env("TALLY_FEISHU_APP_ID"),
            feishu_app_secret=_env("TALLY_FEISHU_APP_SECRET"),
            feishu_verification_token=_env("TALLY_FEISHU_VERIFICATION_TOKEN"),
            feishu_encrypt_key=_env("TALLY_FEISHU_ENCRYPT_KEY"),
            slack_signing_secret=_env("TALLY_SLACK_SIGNING_SECRET"),
            slack_bot_token=_env("TALLY_SLACK_BOT_TOKEN"),
            relay_base_url=_env("TALLY_RELAY_BASE_URL"),
            relay_key=_env("TALLY_RELAY_API_KEY"),
            relay_model=_env("TALLY_RELAY_MODEL", "gemini-2.5-flash"),
            relay_cheap_model=_env("TALLY_RELAY_CHEAP_MODEL",
                                   "gemini-2.5-flash-lite"),
            relay_judge_model=_env("TALLY_RELAY_JUDGE_MODEL", "glm-5"),
            gemini_key=_env("TALLY_GEMINI_API_KEY"),
            glm_key=_env("TALLY_GLM_API_KEY"),
            groq_key=_env("TALLY_GROQ_API_KEY"),
            cerebras_key=_env("TALLY_CEREBRAS_API_KEY"),
            siliconflow_key=_env("TALLY_SILICONFLOW_API_KEY"),
            strong_key=_env("TALLY_STRONG_API_KEY"),
            strong_base_url=_env("TALLY_STRONG_BASE_URL"),
            strong_model=_env("TALLY_STRONG_MODEL"),
        )
    ...
    def api_security_warnings(self) -> list[str]:
        ...
        if self.exposed_beyond_localhost and not self.api_token:
            warnings.append(
                f"API is bound to {self.api_host} with no TALLY_API_TOKEN set. "
        ...
        if self.api_workers > 1 and not self.redis_url:
            warnings.append(
                f"api_workers={self.api_workers} with no TALLY_REDIS_URL. "
    ...
    def require_sec_user_agent(self) -> str:
        ...
        if not self.sec_user_agent:
            raise RuntimeError(
                "TALLY_SEC_USER_AGENT is required. SEC's fair-access policy demands a "
