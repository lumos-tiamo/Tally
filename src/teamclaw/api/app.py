...
from teamclaw.api.deps import AppState
from teamclaw.api.routes import agents, channels, insight, runs
...
def create_app(
    *,
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        app.state.teamclaw = state

        if seed:
            from teamclaw.store import seed_default_agents

            created = seed_default_agents(state.store)
            if created:
                print(f"[teamclaw] seeded {len(created)} agent(s): "
                      f"{', '.join(a.name for a in created)}", flush=True)

        for warning in cfg.api_security_warnings() + state.session.warnings():
            print(f"[teamclaw] WARNING {warning}", flush=True)
        try:
            yield
        finally:
            state.shutdown()
    ...
    async def require_token(request: Request, call_next):  # noqa: ANN001, ANN202
        ...
        if (token
            ...
            presented = (request.headers.get("x-teamclaw-token")
                         or request.headers.get("authorization", "").removeprefix("Bearer ").strip())
            if presented != token:
                return JSONResponse(
                    {"detail": "missing or invalid X-Teamclaw-Token",
    ...
    def healthz(request: Request) -> dict[str, Any]:
        state: AppState = request.app.state.teamclaw
...
def seed_if_empty(app: FastAPI) -> list[str]:
    ...
    state: AppState = app.state.teamclaw
