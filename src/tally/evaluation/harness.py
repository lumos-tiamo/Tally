...
invocation of ``tally eval`` with no credentials configured reported
...
from tally.orchestration.agent import RunResult
...
class Harness:
    ...
    def run(
        ...
        *,
        ...
        """Score every case, optionally several at a time.

        Concurrency is a practical necessity rather than a nicety: a measured L1
        case takes about six minutes, so seven arms over thirty cases is a day of
        wall clock in series. It is safe because a case shares nothing mutable —
        each builds its own router, workspace and tracer, the SEC client's rate
        limiter is thread-safe, and the completion cache writes atomically.

        **Results keep corpus order regardless of completion order.** An eval
        whose row order depends on which container finished first is not
        reproducible, and diffing two arms would become guesswork.
        """
        selected = self.cases[:limit] if limit else self.cases
        out = EvalRun(
            arm=self.arm, level=self.level,
            conditions={**self.conditions, "workers": workers},
            dataset_digest=dataset_digest(selected),
        )

        def one(case: GroundTruthCase) -> CaseOutcome:
            try:
                result = runner(case, self.level)
            except Exception as exc:  # noqa: BLE001 - one bad case must not stop the arm
                return CaseOutcome(case_id=case.case_id, level=self.level,
                                   finished=False,
        ...
        def announce(done: int, case: GroundTruthCase, outcome: CaseOutcome) -> None:
            headline = next(
                (m.rate for name, m in outcome.metrics.items()
                 if name.endswith("@strict") or name == "signal_recall"), None)
            score = f"{headline:.3f}" if headline is not None else "—"
            print(f"[{done:>3}/{len(selected)}] {case.case_id:<14} "
                  f"{'ok ' if outcome.finished else 'inc'} {score} "
                  f"{outcome.run.tokens_total:>8,} tok"
                  + (f"  {outcome.error[:48]}" if outcome.error else ""), flush=True)
        ...
        with ThreadPoolExecutor(max_workers=workers,
                                thread_name_prefix="tally-eval") as pool:
            futures = {pool.submit(one, case): case for case in selected}
            for done, future in enumerate(as_completed(futures), 1):
                case = futures[future]
                outcome = future.result()
                results[case.case_id] = outcome
                if progress:
                    announce(done, case, outcome)
        # Corpus order, not completion order.
        out.outcomes = [results[c.case_id] for c in selected if c.case_id in results]
        return out
