...
DEFAULT_IMAGE = "tally-sandbox:latest"
...
def _bootstrap(tools_path: str | None, rpc_dir: str | None) -> str:
    ...
    if rpc_dir:
        parts.append(f"_o.environ.setdefault('TALLY_RPC_DIR', {rpc_dir!r})")
    return "; ".join(parts)
...
class DockerSandbox:
    ...
    def mount_works(self) -> bool:
        ...
        sentinel = self.workspace / ".tally-mount-probe"
                ...
                try:
                    proc = subprocess.run(
                        ["docker", "run", "--rm", "--network", "none",
                         ...
                         self.image, "cat", "/probe/.tally-mount-probe"],
...
class LocalSandbox:
    ...
    def run(self, code: str, *, limits: SandboxLimits | None = None) -> ExecResult:
        ...
        if self.datasets is not None:
            env["TALLY_DATASETS"] = str(self.datasets.resolve())
