...
from teamclaw.session import SessionStore
from teamclaw.store import AgentDef, AgentStore, build_engine, init_schema
...
def build_spec_for(agent: AgentDef, workspace: Workspace,
    ...
    """
    from teamclaw.scenarios import SCENARIOS
...
class AppState:
    ...
    def build(cls, *, cfg: Settings | None = None, db_path: Path | None = None,
        ...
        from teamclaw.api.runs import RunManager
        from teamclaw.models.registry_default import build_registry

        cfg = cfg or load_settings()
        engine = build_engine(db_path=db_path or (cfg.paths.data / "teamclaw.db"))
    ...
    def health(self) -> dict[str, Any]:
        ...
        from teamclaw.models.registry_default import describe_registry
...
def app_state(request: Request) -> AppState:
    return request.app.state.teamclaw
